...
function validTimestamp(value: unknown, fallbackIndex: number): string {
  ...
  if (timeOnly)
    return `2026-08-31T${timeOnly[1]}:${timeOnly[2]}:${timeOnly[3] ?? '00'}.000Z`;
  return new Date(Date.UTC(2026, 7, 31, 12, 0, fallbackIndex)).toISOString();
}

function cleanService(value: unknown): string {
  return (
    stringValue(value, 'unknown-service')
      .trim()
      .replace(/[^a-zA-Z0-9_.-]/g, '-') || 'unknown-service'
  );
}
...
function parseTextLine(line: string, index: number): LogEntry {
  const timestamp = line.match(
    /\d{4}-\d{2}-\d{2}[T ][0-9:.+-]+|\b\d{2}:\d{2}:\d{2}\b/,
  )?.[0];
  const level = line.match(
    /\b(DEBUG|INFO|WARN(?:ING)?|ERROR|ERR|FATAL|CRITICAL)\b/i,
  )?.[0];
  const explicitService = line.match(
    /(?:service|app|component)=([\w.-]+)/i,
  )?.[1];
  const bracketService = line.match(
    /\[([\w.-]+(?:service|api|worker|db|gateway))\]/i,
  )?.[1];
  const tokenService = line.match(
    /\b([\w.-]+-(?:service|api|worker|gateway))\b/i,
  )?.[1];
  const message = line
    .replace(timestamp ?? '', '')
    .replace(level ?? '', '')
  ...
  return fromRecord(
    {
      timestamp,
      level,
      service: explicitService ?? bracketService ?? tokenService,
      message,
    },
    line,
    index,
...
export function parseLogContent(content: string, filename: string): LogEntry[] {
  ...
  if (extension === 'csv') {
    ...
    if (rows.length < 2)
      throw new Error('CSV files need a header row and at least one log row.');
...
function categoryFor(message: string): string {
  ...
  if (
    /nullpointer|null pointer|cannot read propert|undefined is not/.test(text)
  )
    return 'null-pointer';
  ...
  if (/unauthori[sz]ed|invalid token|authentication/.test(text))
...
function titleFor(category: string): string {
  const titles: Record<string, string> = {
    'checkout-slo': 'Checkout error rate exceeded SLO',
    'payment-failure': 'Checkout and payment failures',
    'null-pointer': 'Null pointer exceptions',
    'connection-refused': 'Connection refused errors',
    timeout: 'Request timeout spike',
...
function timelineTitle(entry: LogEntry): {
  title: string;
  type: TimelineEvent['type'];
} {
  const text = entry.message.toLowerCase();
  if (/deploy|release|version/.test(text))
    return { title: 'Deployment completed', type: 'deploy' };
  if (/alert|pager|slo/.test(text))
    return { title: 'Alert triggered', type: 'alert' };
  if (entry.level === 'fatal' || entry.level === 'error')
    return { title: entry.message.slice(0, 62), type: 'critical' };
  if (/latency|connections|cpu|memory|increased|spike/.test(text))
    return { title: entry.message.slice(0, 62), type: 'signal' };
  ...
  return [...groups.entries()]
    .map(([key, logs], index) => {
      ...
      const trigger = triggerLog
        ? triggerLog.message.slice(0, 54)
        : 'No correlated change found';
      const maximumLevel = logs.reduce(
        (max, entry) =>
          levelOrder[entry.level] > levelOrder[max] ? entry.level : max,
        logs[0].level,
      );
      const severity =
        levelOrder[maximumLevel] >= 3
          ? 'critical'
          : maximumLevel === 'warn'
            ? 'warning'
            : 'info';
      const uniqueTimeline = nearby
        .filter(
      ...
      const timeline = uniqueTimeline.map((entry) => {
        ...
        return {
          ...
          title: event.title,
          detail: `${entry.service} · ${entry.level.toUpperCase()}`,
          type: event.type,
        };
      });
      const fingerprint = `${service}:${category}`;
      const confidence = triggerLog
        ? Math.min(96, 72 + Math.round(Math.log2(logs.length + 1) * 5))
        : Math.min(74, 48 + logs.length * 3);
      return {
        id: String(100 + index),
