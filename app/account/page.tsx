...
export default function AccountPage() {
  ...
  useEffect(() => {
    ...
    void fetch('/api/account')
      .then((r) => r.json() as Promise<{emailConfigured:boolean;user:unknown}>)
      .then((d) => {setConfigured(d.emailConfigured);setSignedIn(Boolean(d.user));})
  ...
  const input =
    'mt-2 w-full border border-[#46564c] bg-[#030504] p-3 text-base';
  return (
    <main className="min-h-screen bg-[#030504] px-5 py-12 text-[#e8eee9]">
      <div className="mx-auto max-w-md border-2 border-[#344139] bg-[#090d0a] p-6 sm:p-8">
        <Link className="font-black text-[#d6ff00]" href="/">
          CRASHLENS
        </Link>
        ...
        <form onSubmit={submit} className="mt-6 space-y-4">
          ...
          {['login', 'signup', 'reset'].includes(mode) && (
            <label className="block text-sm">
              ...
              />
              {mode !== 'login' && (
                <span className="mt-2 block text-sm text-[#91a097]">
          ...
          {message && (
            <output className="block border border-[#d6ff00] p-3 text-sm">
              {message}
            </output>
          )}
          <button
            disabled={busy}
            className="w-full bg-[#d6ff00] p-3 font-bold text-black disabled:opacity-50"
          >
            {busy
              ? 'Please wait…'
              : mode === 'login'
        ...
        <nav className="mt-6 flex flex-wrap gap-4 text-sm text-[#00d9ff]">
          {mode !== 'login' && (
        ...
        <a
          ...
          target="_top"
          className="mt-6 block border-t border-[#344139] pt-4 text-sm text-[#91a097]"
        >
