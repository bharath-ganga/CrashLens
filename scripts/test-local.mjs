...
try {
  ...
  ).run(id, email, 'QA fixture', await hashPassword(password));
  const verify = randomToken();
  db.prepare('INSERT INTO account_tokens VALUES (?,?,?,?)').run(
    await tokenHash(verify),
    id,
    'verify',
    Date.now() + 60000,
  );
  await account({ action: 'verify', token: verify }, 200);
  await account({ action: 'verify', token: verify }, 400);
  await account({ action: 'login', email, password: 'wrong' }, 401);
  const login = await account({ action: 'login', email, password }, 200);
  session = login.r.headers.get('set-cookie').split(';')[0];
  assert.ok(login.r.headers.get('set-cookie').includes('HttpOnly'));
  const me = await fetch(`${base}/api/account`, {
  ...
  const create = await fetch(`${base}/api/monitors`, {
    ...
    body: JSON.stringify({
      ...
      assertionValue: 'Example Domain',
      headers: '{}',
      tags: 'qa, public',
    }),
  });
  assert.equal(create.status, 201);
  monitorId = (await create.json()).id;
  const list = await fetch(`${base}/api/monitors`, {
    headers: { Cookie: session },
  }).then((r) => r.json());
  assert.equal(list.monitors.length, 1);
  assert.equal(list.monitors[0].id, monitorId);
  assert.equal(list.monitors[0].method, 'GET');
  assert.equal(list.monitors[0].assertion_type, 'contains');
  assert.equal(list.schedulerActive, false);
  const pause = await fetch(`${base}/api/monitors`, {
    method: 'POST',
    headers: {
      Origin: base,
      Cookie: session,
    ...
    body: JSON.stringify({ action: 'pause', id: monitorId }),
  });
  assert.equal(pause.status, 200);
  const csrf = await fetch(`${base}/api/account`, {
    method: 'POST',
    headers: {
  ...
  db.prepare('INSERT INTO account_tokens VALUES (?,?,?,?)').run(
    await tokenHash(reset),
    id,
    'reset',
    Date.now() + 60000,
  );
  const updated = randomToken();
  await account({ action: 'reset', token: reset, password: updated }, 200);
  await account({ action: 'reset', token: reset, password: updated }, 400);
  const expired = await fetch(`${base}/api/account`, {
    headers: { Cookie: session },
  }).then((r) => r.json());
  assert.equal(expired.user, null);
  const next = await account(
    { action: 'login', email, password: updated },
    200,
  );
  session = next.r.headers.get('set-cookie').split(';')[0];
  await account({ action: 'logout' }, 200);
  assert.equal(
    db
      .prepare(
        "SELECT COUNT(*) AS n FROM email_outbox WHERE recipient=? AND event_key LIKE 'login:%'",
      )
      .get(email).n,
    2,
  );
  const unauthorized = await fetch(`${base}/api/monitor-tick`, {
    method: 'POST',
  });
  ...
  if (monitorId)
    db.prepare('DELETE FROM uptime_checks WHERE monitor_id=?').run(monitorId);
  ...
  db.prepare('DELETE FROM monitor_projects WHERE created_by=?').run(id);
